...
def show_venue(venue_id):
  ...
  print('Hi, show_venue')
  # talentVenue = False
  # if venue.seeking_talent.lower() == 'true' :
  #   talentVenue = True
  #select no. of past shows per venue
  #pastShowsPerVenue=Show.query.with_entities(func.count(Show.id)).filter(Show.start_time < datetime.datetime.now()).group_by(Show.venue_id).having(Show.venue_id==venue_id).all()
  #select no. of upcoming shows per venue
  #upcomingShowsPerVenue=Show.query.with_entities(func.count(Show.id)).filter(Show.start_time >= datetime.datetime.now()).group_by(Show.venue_id).having(Show.venue_id==venue_id).all()
  pastShowData=[]
  upcomingShowData=[]
...
def edit_artist_submission(artist_id):
  print('Hi, edit_artist_submission')
...
def edit_venue_submission(venue_id):
  print('Hi, edit_venue_submission')
  # TODO: take values from the form submitted, and update existing
...
def create_artist_form():
  print('Hi, create_artist_form')
  form = ArtistForm()
  return render_template('forms/new_artist.html', form=form)

@app.route('/artists/create', methods=['POST'])
def create_artist_submission():
  print('Hi, create_artist_submission')
  ...
  try:
    ...
    #genres
    genrestxt=''
    print('create_artist_submission: request.form ')
    f = request.form
    for key in f.keys():
      for value in f.getlist(key):
        print (key,":",value)
    ...
    print('create_artist_submission: after add')
    db.session.commit()
    print('create_artist_submission: after commit')
    # on successful db insert, flash success
    flash('Artist ' + request.form['name'] + ' was successfully listed!')
    print('create_artist_submission: after flash')
  except:
...
def not_found_error(error):
  print('Hi, not_found_error')
  return render_template('errors/404.html'), 404

@app.errorhandler(500)
def server_error(error):
  print('Hi, server_error')
